...
export default function Reviews({ content, reviews, imageBg }: ReviewsProps) {
  ...
  return (
    <section 
        id="reviews" 
        className="relative py-12 md:py-20 bg-card overflow-hidden"
    >
      {imageBg && imageBg.src && (
        <Image
          src={imageBg.src}
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {reviewsToShow.map((testimonial, index) => (
            ...
            >
              <ReviewCard review={testimonial} truncate={true} />
            </div>
          ))}
        </div>

        {reviews.length > 4 && (
          <div className="text-center mt-12 opacity-0 animate-fade-in-up [animation-delay:700ms]">
